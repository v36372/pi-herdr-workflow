...
test("pi --extension loads /workflow list", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    assert.match(result.stdout, /pi-herdr-workflows launch/);
    assert.match(result.stdout, /hello/);
    assert.match(result.stdout, /src\/extension\/index\.ts/);
  } finally {
...
test("pi --extension runs compute and shell example workflows", { timeout: 60_000 }, async () => {
  ...
  assert.equal(hello.code, 0, hello.stderr + hello.stdout);
  assert.match(hello.stdout, /Workflow hello completed/);
  assert.match(hello.stdout, /hello, tin/);
  ...
  assert.equal(pipeline.code, 0, pipeline.stderr + pipeline.stdout);
  assert.match(pipeline.stdout, /Workflow 01-compute-pipeline completed/);
  assert.match(pipeline.stdout, /pi-herdr/);

  const facts = await runPi({
    prompt: `/workflow ${path.join("examples", "demo", "02-shell-facts.workflow.ts")} --input-json {"label":"build"}`,
  });
  assert.equal(facts.code, 0, facts.stderr + facts.stdout);
  assert.match(facts.stdout, /Workflow 02-shell-facts completed/);
  assert.match(facts.stdout, /shell-ok/);
  ...
  assert.equal(timeout.code, 0, timeout.stderr + timeout.stdout);
  assert.match(timeout.stdout, /Workflow 03-timeout-budget completed/);

  const rejoin = await runPi({
    prompt: `/workflow ${path.join("examples", "demo", "05-decision-rejoin.workflow.ts")} --input-json {"route":"right"}`,
  });
  assert.equal(rejoin.code, 0, rejoin.stderr + rejoin.stdout);
  assert.match(rejoin.stdout, /took right path/);

  const repair = await runPi({
    prompt: `/workflow ${path.join("examples", "demo", "07-repair-outcome.workflow.ts")} --input-json {"mode":"fail"}`,
  });
  assert.equal(repair.code, 0, repair.stderr + repair.stdout);
  assert.match(repair.stdout, /"status": "repaired"/);
});
...
test("pi --extension spawn interface runs agent example workflows via stub model", { timeout: 180_000 }, async () => {
  ...
  assert.equal(matrix.code, 0, matrix.stderr + matrix.stdout);
  assert.match(matrix.stdout, /Workflow 04-spawn-matrix completed/);
  assert.match(matrix.stdout, /"echo": "ping"/);
  ...
  assert.equal(echo.code, 0, echo.stderr + echo.stdout);
  assert.match(echo.stdout, /Workflow echo completed/);
  assert.match(echo.stdout, /"reply":/);
  ...
  assert.equal(relay.code, 0, relay.stderr + relay.stdout);
  assert.match(relay.stdout, /Workflow relay completed/);
  assert.match(relay.stdout, /"shout": "OCEAN"/);
});
